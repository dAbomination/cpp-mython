...
namespace ast {	
	...
	// Исключение для Return
	class ReturnException : public std::runtime_error {
	public:		

		ReturnException(runtime::ObjectHolder&& statement)
			: std::runtime_error(""), statement_(std::move(statement)) {
		}

		runtime::ObjectHolder GetStatement() const {
			return statement_;
		}
	private:
		runtime::ObjectHolder statement_;
	};
	...
	*/
	...
	public:
		...
		explicit Compound(Args&&... args) {	
			(stmts_.push_back(std::move(args)), ...);
		}
